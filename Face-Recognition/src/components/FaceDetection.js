...
const FaceDetection = ({ data, available }) => {
    ...
    setInterval(async () => {
      ...
      if (detection) {
        ...
        Object.keys(resizedDetections.expressions).forEach((key) => {
          ...
          if (
            resizedDetections.expressions[key] > minConfidence &&
            initialized === false
          )
            if (!objExpressionDescriptors.hasOwnProperty(key)) {
              ...
              if (hasAllExpressions()) {
                ...
                const descriptors = [
                  ...Object.values(objExpressionDescriptors),
                ];
  ...
  function loadLabeledImages() {
    const labels = ["Aman Shaikh"]; // for WebCam
    return Promise.all(
  ...
  const addStudent = async (descriptors) => {
    ...
    if (db != null) {
      let label = await prompt("Enter your username", "Aman");
  ...
  function List() {
    return (
      <ol>
        {() => Present.map((name) => <li key={name}>{name} is present.</li>)}
      </ol>
    );
  }
